(ns chime.main
  "Kotodama WASM entrypoint for the Chime clean-room actor (L4) — Clojure port.
  ...
  facts (`chime.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"chime.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "chime-compat" :tier tier :entities entities} 200])
